...
do
  ...
  elif [ "$var" == "--P4EST" -o "$var" == "-p4est" ]; then
    BUILD_P4EST=1
...
if [ $BUILD_METIS == 0 -a $BUILD_P4EST == 0 ]; then
  BUILD_METIS=1
  BUILD_P4EST=1
fi
...
# =================================================================== #
COMPILE_FAIL=0
INSTALL_P4EST_DIRECTORY=${INSTALL_3PL_DIRECTORY}/p4est
if [ ${BUILD_P4EST} -eq 1 ]; then
  echo " "
  echo -e "${mC} ==== Building p4est ==== ${eC}"
  echo " Compiling Options:"
  echo "        Build Type: ${BUILD_TYPE}"
  echo "  Install Location: ${INSTALL_P4EST_DIRECTORY}"
  echo " "
  echo "                CC: ${CC}"
  echo "               CXX: ${CXX}"
  echo "                FC: ${FC}"
  ...
  cd ${SOURCES_3PL_DIRECTORY}/p4est
  rm -rf ${INSTALL_P4EST_DIRECTORY}

  git submodule init
  git submodule update

  ./bootstrap
  ./configure --prefix=${INSTALL_P4EST_DIRECTORY}                   \
  ...
  if [ ! -d "${INSTALL_P4EST_DIRECTORY}" ]; then
    echo "ERROR:"
    echo "${INSTALL_P4EST_DIRECTORY} does not exist."
    COMPILE_FAIL=1
  fi
  ...
  if [ ${COMPILE_FAIL} == 0 ]; then
    echo " "
    echo         "========================="
    echo -e "${gC} p4est build successful! ${eC}"
    echo         "========================="
    echo " "
  else
    echo " "
    echo         "==========================="
    echo -e "${rC} p4est build FAILED! ${eC}"
    echo         "==========================="
    echo " "
    exit 1
  fi
