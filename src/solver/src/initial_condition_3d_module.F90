...
!     
! \file   initial_condition_3d_module.F90
! \author akirby
!
! Created on August 31, 2018, 11:24 AM
!


module initial_condition_3d_module
    implicit none
    contains
    
    
    subroutine initial_condition_3d(initial_condition,unstructured,nfields,nelem_subgrid,h,cell_geom,Q,&
        ...
        real(dp)    :: RC,RT0pRC
        real(dp)    :: C_0,V_0
        real(dp)    :: u_in,v_in,w_in
        real(dp)    :: rhou,rhov,rhow,rhoe
        real(dp)    :: time

        real(dp)    :: node_natural(3),node(3)
        ...
        w_in        = V_0*sin(beta*pi/180.0_dp)


        rhou = density*u_in
        rhov = density*v_in
        rhow = density*w_in
        rhoe = (pressure * oneOgm1)  + half*density*(u_in*u_in + v_in*v_in + w_in*w_in)
            ...
            case(1) !isentropic vortex
                ...
                do j = 1,nelem_subgrid
                    ...
                    do i = 1,nelem_subgrid
                        ...
                        call bilinear_node_3d(cell_geom,node_natural,node)
                        ...
                        Q(3,i,j,1) = rho*v
                        Q(4,i,j,1) = 0.0_dp
                        Q(5,i,j,1) = p*oneOgm1 + half*rho*(u*u+v*v+w*w)

                    end do

                end do

                do k = 2,nelem_subgrid
                    Q(:,:,:,k) = Q(:,:,:,1)
                end do
