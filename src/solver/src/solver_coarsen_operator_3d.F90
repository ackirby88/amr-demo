!>     
!> \file   solver_coarsen_operator_3d.F90
!> \author akirby
!> \ingroup solver_group
!> \brief 3D coarsen operators
!> 
!> Created on September 6, 2018, 10:43 AM
!>


!> 3D coarsen operator: Unweighted Average
!>
...
!>
subroutine solver_coarsen_operator_3d(unstructured,nfields,nelem_subgrid,cell_type,h,geom,&
                                      uc1,uc2,uc3,uc4,uc5,uc6,uc7,uc8,soln) bind(C)
    use iso_c_binding
    implicit none
    ...
    real(c_double),intent(in)   :: uc5(nfields*nelem_subgrid*nelem_subgrid)
    real(c_double),intent(in)   :: uc6(nfields*nelem_subgrid*nelem_subgrid)
    real(c_double),intent(in)   :: uc7(nfields*nelem_subgrid*nelem_subgrid)
    real(c_double),intent(in)   :: uc8(nfields*nelem_subgrid*nelem_subgrid)
    real(c_double),intent(out)  :: soln(nfields*nelem_subgrid*nelem_subgrid)


    !average all the values together
    soln = uc1 + uc2 + uc3 + uc4 + uc5 + uc6 + uc7 + uc8
    soln = 0.125*soln
