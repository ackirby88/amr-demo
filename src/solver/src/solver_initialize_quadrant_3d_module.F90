...
!>     
!> \file   solver_initialize_quadrant_3d_module.F90
!> \author akirby
!> \ingroup solver_group
!> \brief 3D initialize quadrant
!> 
!> Created on August 30, 2018, 9:38 AM
!>

module solver_initialize_quadrant_3d_module
    implicit none
    contains


    !> 3D initialize quadrant and boundary conditions
    !> 
    ...
    !>
    subroutine solver_initialize_quadrant_3d(initial_condition,unstructured,nfields,nelem_subgrid,cell_type,h,cell_geom,Q,&
                                             mach,alpha,beta,gamma,density,pressure)
        use my_kinddefs
        use initial_boundary_3d_module
        use initial_condition_3d_module
        implicit none
        ...
        real(dp),   intent(out)     :: Q(nfields,nelem_subgrid,nelem_subgrid,nelem_subgrid)
        ...
        call initial_boundary_3d(unstructured,cell_geom,cell_type)
        call initial_condition_3d(initial_condition,unstructured,nfields,nelem_subgrid,h,cell_geom,Q,&
                                  mach,alpha,beta,gamma,density,pressure)
