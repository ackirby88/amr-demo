...
!>     
!> \file   solver_residual_bc_structured_2d_module.F90
!> \author akirby
!> \ingroup solver_group
!> \brief 2D boundary condition residual
!> 
!> Created on September 4, 2018, 10:18 AM
!>

module solver_residual_bc_structured_2d_module
    implicit none
    contains
    

    !> 2D boundary condition residual
    !> 
    ...
    !>
    subroutine solver_residual_bc_structured_2d(nfields,nelem_subgrid,side,cell_type,level,bc,h_base,xlo,ylo,zlo,soln,res)
        use my_kinddefs
        use bc_2d_module
        use flux_2d_module
        ...
        real(dp),   intent(in)      :: soln(nfields,nelem_subgrid,nelem_subgrid)
        ...
        do n = 1,nelem_subgrid
            ...
            call bc_2d(bc,normal_vec,soln(:,i,j),Q_bc)
            call flux_bc_2d(Q_bc,normal_vec,flux)
